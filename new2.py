...
def create_table_from_df(engine, df, table_name, primary_key_columns):
    ...
    # Get the original data types from the DataFrame
    original_data_types = {col: str(df[col].dtype) for col in df.columns}
    
    # Map the original data types to MySQL-compatible types
    columns = ", ".join([f"{col} {mysql_data_types[original_data_types[col]]}" for col in df.columns])
...
def insert_data_ignore_duplicates(engine, df, table_name, primary_key_columns, other_columns):
    """
    Modified function to insert data from a pandas DataFrame into a given table,
    ignoring duplicates based on primary key columns.
    """
    # Prepare the INSERT statement with ON DUPLICATE KEY UPDATE
    insert_query = f"""
        INSERT INTO {table_name} ({', '.join(primary_key_columns + other_columns)})
        VALUES ({', '.join(['%s'] * len(primary_key_columns + other_columns))})
        ON DUPLICATE KEY UPDATE
        {', '.join([f"{col} = VALUES({col})" for col in other_columns])}
    """
    
    # Execute the prepared query for each row in the DataFrame
    for _, row in df.iterrows():
        params = tuple(row[col] for col in primary_key_columns + other_columns)
        with engine.connect() as connection:
            try:
...
def extract_other_columns(df, primary_key_columns):
    """
    Extracts other columns from a DataFrame by excluding the primary key columns.

    Parameters:
    - df: pandas DataFrame containing the data.
    - primary_key_columns: List of column names that serve as primary keys.

    Returns:
    - A list of column names that are not primary keys.
    """
    all_columns = df.columns.tolist()
    other_columns = [col for col in all_columns if col not in primary_key_columns]
...
def process_anc_register_data():
    ...
    # Dynamically determine primary key and other columns
    primary_key_columns = ['anc_id', 'registered_date']
    all_columns = anc_register_df.columns.tolist()
    other_columns = [col for col in all_columns if col not in primary_key_columns]
    #other_columns = extract_other_columns(anc_register_df, primary_key_columns)

    insert_data_ignore_duplicates(target_engine, anc_register_df, 'anc_register', primary_key_columns, other_columns)
...
def process_anc_booked_register_data():
    ...
    # Dynamically determine primary key and other columns
    primary_key_columns = ['anc_id', 'booked_date']
    all_columns = anc_booked_register_df.columns.tolist()
    other_columns = [col for col in all_columns if col not in primary_key_columns]

    insert_data_ignore_duplicates(target_engine, anc_booked_register_df, 'anc_booked_register', primary_key_columns, other_columns)
...
def process_hts_register_data():
    ...
    # Dynamically determine primary key and other columns
    primary_key_columns = ['hts_id', 'hts_date']
    all_columns = hts_register_df.columns.tolist()
    other_columns = [col for col in all_columns if col not in primary_key_columns]

    insert_data_ignore_duplicates(target_engine, hts_register_df, 'hts_register', primary_key_columns, other_columns)
...
def process_hiv_positive_person_data():
    ...
    # Dynamically determine primary key and other columns
    primary_key_columns = ['investigation_order_id', 'visit_date']
    all_columns = hiv_positive_person_df.columns.tolist()
    other_columns = [col for col in all_columns if col not in primary_key_columns]

    insert_data_ignore_duplicates(target_engine, hiv_positive_person_df, 'hiv_positive_person', primary_key_columns, other_columns)
...
def process_hts_screening_register_data():
    ...
    # Dynamically determine primary key and other columns
    primary_key_columns = ['hts_screening_id', 'patient_id']
    all_columns = hts_screening_df.columns.tolist()
    other_columns = [col for col in all_columns if col not in primary_key_columns]

    insert_data_ignore_duplicates(target_engine, hts_screening_df, 'hts_screening_register', primary_key_columns, other_columns)
...
def process_hts_register_test_data():
    ...
    # Dynamically determine primary key and other columns
    primary_key_columns = ['order_test_id', 'hts_id']
    all_columns = hts_test_df.columns.tolist()
    other_columns = [col for col in all_columns if col not in primary_key_columns]

    insert_data_ignore_duplicates(target_engine, hts_test_df, 'hts_register_test', primary_key_columns, other_columns)
...
def process_art_register_data():
   ...
   # Dynamically determine primary key and other columns
   primary_key_columns = ['art_id', 'person_id']
   all_columns = art_df.columns.tolist()
   other_columns = [col for col in all_columns if col not in primary_key_columns]

   insert_data_ignore_duplicates(target_engine, art_df, 'art_register', primary_key_columns, other_columns)
...
def process_art_register_followup_data():
   ...
   # Dynamically determine primary key and other columns
   primary_key_columns = ['visit_id', 'person_id']
   all_columns = art_followup_df.columns.tolist()
   other_columns = [col for col in all_columns if col not in primary_key_columns]

   insert_data_ignore_duplicates(target_engine, art_followup_df, 'art_register_followup', primary_key_columns, other_columns)
...
def process_art_transfer_in_register_data():
    ...
    # Dynamically determine primary key and other columns
    primary_key_columns = ['id', 'art_id']
    all_columns = art_transfer_in_df.columns.tolist()
    other_columns = [col for col in all_columns if col not in primary_key_columns]
    
    insert_data_ignore_duplicates(target_engine, art_transfer_in_df, 'art_transfer_in_register', primary_key_columns, other_columns)
...
def process_art_transfer_out_register_data():
    ...
    # Dynamically determine primary key and other columns
    primary_key_columns = ['art_transfer_out_register_id', 'art_id']
    all_columns = art_transfer_out_df.columns.tolist()
    other_columns = [col for col in all_columns if col not in primary_key_columns]

    insert_data_ignore_duplicates(target_engine, art_transfer_out_df, 'art_transfer_out_register', primary_key_columns, other_columns)
...
def process_art_visit_register_data():
    ...
    # Dynamically determine primary key and other columns
    primary_key_columns = ['art_visit_id', 'visit_date']
    all_columns = art_visit_df.columns.tolist()
    other_columns = [col for col in all_columns if col not in primary_key_columns]

    insert_data_ignore_duplicates(target_engine, art_visit_df, 'art_visit_register', primary_key_columns, other_columns)
...
def process_art_who_stage_register_data():
    ...
    # Dynamically determine primary key and other columns
    primary_key_columns = ['art_stage_id', 'visit_date']
    all_columns = art_who_stage_df.columns.tolist()
    other_columns = [col for col in all_columns if col not in primary_key_columns]

    insert_data_ignore_duplicates(target_engine, art_who_stage_df, 'art_who_stage_register', primary_key_columns, other_columns)
...
def process_art_ipt_status_register_data():
    ...
    # Dynamically determine primary key and other columns
    primary_key_columns = ['art_ipt_status_id', 'visit_date']
    all_columns = art_tpt_status_df.columns.tolist()
    other_columns = [col for col in all_columns if col not in primary_key_columns]
    
    insert_data_ignore_duplicates(target_engine, art_tpt_status_df, 'art_ipt_status_register', primary_key_columns, other_columns)
...
def process_art_current_status_register_data():
    ...
    # Dynamically determine primary key and other columns
    primary_key_columns = ['art_status_id', 'visit_date']
    all_columns = art_current_status_df.columns.tolist()
    other_columns = [col for col in all_columns if col not in primary_key_columns]

    insert_data_ignore_duplicates(target_engine, art_current_status_df, 'art_current_status_register', primary_key_columns, other_columns)
...
def process_viral_load_data():
    ...
    # Dynamically determine primary key and other columns
    primary_key_columns = ['laboratory_investigation_id', 'date']
    all_columns = viral_load_df.columns.tolist()
    other_columns = [col for col in all_columns if col not in primary_key_columns]

    insert_data_ignore_duplicates(target_engine, viral_load_df, 'viral_load_investigation', primary_key_columns, other_columns)
...
def process_tpt_screening_data():
    ...
    # Dynamically determine primary key and other columns
    primary_key_columns = ['patient_tpt_screening_id', 'date']
    all_columns = tpt_screening_df.columns.tolist()
    other_columns = [col for col in all_columns if col not in primary_key_columns]

    insert_data_ignore_duplicates(target_engine, tpt_screening_df, 'tpt_screening', primary_key_columns, other_columns)
...
def process_anc_visit_data():
    ...
    # Dynamically determine primary key and other columns
    primary_key_columns = ['anc_visit_id', 'anc_id']
    all_columns = anc_visit_df.columns.tolist()
    other_columns = [col for col in all_columns if col not in primary_key_columns]

    insert_data_ignore_duplicates(target_engine, anc_visit_df, 'anc_visit', primary_key_columns, other_columns)
...
def process_art_appointment_data():
    ...
    # Dynamically determine primary key and other columns
    primary_key_columns = ['art_appointment_register_id', 'date']
    all_columns = art_appointment_df.columns.tolist()
    other_columns = [col for col in all_columns if col not in primary_key_columns]

    insert_data_ignore_duplicates(target_engine, art_appointment_df, 'art_appointment', primary_key_columns, other_columns)
...
def process_cbs_data():
    ...
    # Dynamically determine primary key and other columns
    primary_key_columns = ['person_id', 'site_id']
    all_columns = cbs_df.columns.tolist()
    other_columns = [col for col in all_columns if col not in primary_key_columns]

    insert_data_ignore_duplicates(target_engine, cbs_df, 'cbs', primary_key_columns, other_columns)
...
def process_art_dsd_group_data():
    ...
    # Dynamically determine primary key and other columns
    primary_key_columns = ['art_dsd_group_id', 'date']
    all_columns = dsd_group_df.columns.tolist()
    other_columns = [col for col in all_columns if col not in primary_key_columns]

    insert_data_ignore_duplicates(target_engine, dsd_group_df, 'art_dsd_group', primary_key_columns, other_columns)
...
def process_art_dsd_client_data():
    ...
    # Dynamically determine primary key and other columns
    primary_key_columns = ['art_dsd_group_id', 'date_joined']
    all_columns = dsd_client_df.columns.tolist()
    other_columns = [col for col in all_columns if col not in primary_key_columns]

    insert_data_ignore_duplicates(target_engine, dsd_client_df, 'art_dsd_group_client', primary_key_columns, other_columns)
...
def process_art_dsd_appointment_data():
    ...
    # Dynamically determine primary key and other columns
    primary_key_columns = ['id', 'appointment_date']
    all_columns = dsd_group_appointment_df.columns.tolist()
    other_columns = [col for col in all_columns if col not in primary_key_columns]

    insert_data_ignore_duplicates(target_engine, dsd_group_appointment_df, 'art_dsd_group_appointment', primary_key_columns, other_columns)

def main():
    # sql_folder = '/home/devoop/Documents/eHR/mutare' 
    sql_files = [f for f in os.listdir(backup_folder) if f.endswith('.sql') and not f.startswith('._')]
    
    for sql_file in sql_files:
        sql_file_path = os.path.join(backup_folder, sql_file)
        load_sql_file_into_mysql(sql_file_path, container_name, 'hivdatabase', 'root')
